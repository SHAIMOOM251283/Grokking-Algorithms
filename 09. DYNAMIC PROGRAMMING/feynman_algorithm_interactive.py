...
def main():
    print("Welcome to the Feynman Algorithm")

    # Input the size of the cost matrix
    n = int(input("Enter the size of the cost matrix: "))

    # Input the cost matrix
    print("Enter the cost matrix (each row on a new line):")
    cost_matrix = []
    for _ in range(n):
        row = list(map(int, input().split()))
        cost_matrix.append(row)

    # Apply the Feynman algorithm
